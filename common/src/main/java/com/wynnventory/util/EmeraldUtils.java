...
public class EmeraldUtils {
    ...
    public static String getFormattedString(int emeralds, boolean appendZeros) {
        ...
        if (emeraldAmounts[3] > 0) {
            ...
            builder.append(emeraldAmounts[3])
                    .append(EmeraldUnits.LIQUID_EMERALD_STX.getSymbol())
                    .append(" ");

            double fractionalLe = emeraldAmounts[2] + emeraldAmounts[1] / 64.0 + emeraldAmounts[0] / 4096.0;
            if (fractionalLe > 0 || appendZeros) {
                builder.append(String.format("%.1f", fractionalLe))
                        .append(EmeraldUnits.LIQUID_EMERALD.getSymbol())
                        .append(" ");
            }
        } else {
